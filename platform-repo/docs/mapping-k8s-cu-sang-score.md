...
| Ingress nginx (host/path/tls) | `type: route` | mỗi host/path = 1 resource route; đa host be/fe → route ở từng workload; TLS: TODO entryPoint websecure |
...
| event-management, otm | be/fe + `mysql` (5.6) + route | mysql 5.6 EOL: `params.image` giữ 5.6/5.7 lúc migrate, nâng cấp sau |
...
5. Trỏ DNS host sang Traefik cụm mới; giữ cụm cũ read-only 1–2 tuần làm rollback.
